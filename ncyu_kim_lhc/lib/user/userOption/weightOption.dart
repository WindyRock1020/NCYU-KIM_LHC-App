...
class _EffectiveLoadWeight extends State<WeightOption> {
  ...
  void _update() {
    setState(() {
      _loadWeightPoints = calculateWeight(gender, _weight);
  ...
  void _loadGender() async {
    ...
    setState(() {
      gender = prefs.getString('${widget.userName}_gender') ?? "None";
      _update();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ProgressBar(
            current: calculateWeight(gender, _weight),
            max: 100,
            barSize: 40,
            labelText: "${_loadWeightPoints.toString().replaceAll(".0", "")}分 / 100分",
            textSize: 20,
          ),
          Expanded(
            child: Align(
