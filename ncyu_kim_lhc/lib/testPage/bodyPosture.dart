...
class BodyPosture extends StatefulWidget {
  ...
  @override
  _BodyPostureState createState() => _BodyPostureState();
}

class _BodyPostureState extends State<BodyPosture> {
  ...
  String endPos = "None";
  String keyPointVidURL = "http://120.110.114.17:8080";
  ...
  _loadURL() async {
    ...
    keyPointVidURL = prefs.getString('uploadUrl') ?? 'http://120.110.114.17:8080';
  }
  ...
  Future<void> _saveBodyPoints() async {
    ...
    await prefs.setDouble('totalBodyPosturePoints', widget.totalBodyPoints);
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          ProgressBar(
            labelText:
                "${widget.totalBodyPoints.toString().replaceAll('.0', '')} 分 / 26 分",
            current: widget.totalBodyPoints,
            textSize: 20*MediaQuery.of(context).textScaleFactor,
            max: 26,
            barSize: 40,
          ),
          Expanded(
